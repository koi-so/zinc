...
This header compiles properly at Wall on Visual Studio, -Wall on gcc, and
-Weverything on clang. Tested most recently on clang 3.8.0, gcc 7.3.1, and both
...
// and does pretend to be gcc. Check it first, as nothing else pretends to be
// clang
...
// Note - there is a __cpp_constexpr test now, but everything prior to VS 2017
// reports incorrect values and this version always supports at least the
// CPLUSPLUS_14 approach
#define CPLUSPLUS_STD CPLUSPLUS_14
...
#define CPLUSPLUS_STD CPLUSPLUS_98
     ...
     SAFEINT_COMPILER == GCC_COMPILER) &&                                      \
    CPLUSPLUS_STD < CPLUSPLUS_11
...
// If the user has an unknown compiler, or just for testing, allow forcing this
// setting
#if !defined CONSTEXPR_SUPPORT

#if defined __cpp_constexpr
// If it is gcc or clang, at least recent versions, then we have -std=c++11 or
// -std=c++14 This won't be set otherwise, but the headers won't compile, either
#if __cpp_constexpr >= 201304L
#define CONSTEXPR_SUPPORT                                                      \
  CONSTEXPR_CPP14 // Clang, gcc, Visual Studio 2017 or later
#elif __cpp_constexpr >= 200704L
#define CONSTEXPR_SUPPORT                                                      \
  CONSTEXPR_CPP11 // Clang, gcc with -std=c++11, Visual Studio 2015
#else
#define CONSTEXPR_SUPPORT CONSTEXPR_NONE
#endif

#else // !defined __cpp_constexpr
// Visual Studio is somehow not playing nice. shows __cpp_constexpr visually as
// defined, but won't compile
...
// Note - this intentionally leaks from the header, doesn't quench the warnings
// otherwise Also disable Spectre mitigation warning
...
// Other compilers might turn these into exceptions, and some users may want to
// not have throw() enabled. In addition, some error handlers may not throw C++
// exceptions, which makes everything no throw. noexcept requires C++11
...
// If it is important to get constexpr for multiplication, then define
// SAFEINT_USE_INTRINSICS 0 However, intrinsics will result in much smaller
// code, and should have better perf
...
// If it is the Visual Studio compiler, then it has to be 64-bit, and not
// ARM64EC
#if SAFEINT_COMPILER == VISUAL_STUDIO_COMPILER
...
// The gcc and clang builtin functions are constexpr, but not the Microsoft
// intrinsics
...
static_assert(-1 == static_cast<int>(0xffffffff),
              "Two's complement signed numbers are required");
...
*                 added extended support for mixed-type operations where
possible
*  Jan 28, 2004 - 1.0.1
*                 changed WCHAR to wchar_t
*                 fixed a construct in two mixed-type assignment overloads that
was
*                 not compiling on some compilers
*                 Also changed name of private method to comply with standards
on
*                 reserved names
*                 Thanks to Niels Dekker for the input
*  Feb 12, 2004 - 1.0.2
*                 Minor changes to remove dependency on Windows headers
*                 Consistently used std::int16_t, std::int32_t and std::int64_t
to ensure
*                 portability
*  May 10, 2004 - 1.0.3
...
*                 Pulled error handler out into function to enable user-defined
replacement
*                 Made internal type of SafeIntException an enum (as per Niels'
suggestion)
*                 Added casts for base integer types (as per Scott Meyers'
suggestion)
*                 Updated usage information - see important new perf notes.
*                 Cleaned up several const issues (more thanks to Niels)
*
*  Oct 1, 2004 - 1.0.5
*                 Added support for SEH exceptions instead of C++ exceptions -
Win32 only
...
*  Nov 5, 2004 - 1.0.6
*                 Implemented new logic in binary operators to resolve issues
with
*                 implicit casts
...
*                 Re-implemented casting operator as a result of removing Value
method
*  Dec 1, 2004 - 1.0.7
*                 Implemented specialized operators for pointer arithmetic
*                 Created overloads for cases of U op= SafeInt. What you do with
U
*                 after that may be dangerous.
*                 Fixed bug in corner case of MixedSizeModulus
*                 Fixed bug in MixedSizeMultiply and MixedSizeDivision with
input of 0
...
*                 Changes needed to support gcc compiler - many thanks to Niels
Dekker
*                 for determining not just the issues, but also suggesting
fixes.
*                 Also updating some of the header internals to be the same as
the upcoming Visual Studio version.
*
*  Jan 16, 2010   64-bit gcc has long == std::int64_t, which means that many of
the existing 64-bit
*                 templates are over-specialized. This forces a redefinition of
all the 64-bit
*                 multiplication routines to use pointers instead of references
for return
*                 values. Also, let's use some intrinsics for x64 Microsoft
compiler to
*                 reduce code size, and hopefully improve efficiency.
*
*  June 21, 2014  Better support for clang, higher warning levels supported for
all 3 primary supported compilers (Visual Studio, clang, gcc). Also started to
converge the code base such that the public CodePlex version will be a drop-in
replacement for the Visual Studio version.

* Feb 12, 2018    Fixed floating point bug
*                 Fix to allow initialization by an enum
*                 Add support for static_assert, make it default to fix compiler
warnings from C_ASSERT on gcc, clang
*                 Changed throw() to noexcept

* March, 2018     Introduced support for constexpr, both the C++11 and C++14
flavors work. The C++14 standard allows for much more thorough usage, and should
be preferred.

* August, 2022    Added support for nodiscard


*  Note about code style - throughout this class, casts will be written using
C-style (T),
*  not C++ style static_cast< T >. This is because the class is nearly always
dealing with integer
*  types, and in this case static_cast and a C cast are equivalent. Given the
large number of casts,
*  the code is a little more readable this way. In the event a cast is needed
where static_cast couldn't
*  be substituted, we'll use the new templatized cast to make it explicit what
...
// If the user has already defined an exception handler, we don't need built-in
...
#define SAFEINT_EXCEPTION_CPP 2
    ...
    !defined SAFE_INT_USE_STDLIB // Win32 structured exceptions
#define SAFEINT_EXCEPTION_METHOD SAFEINT_EXCEPTION_WIN32
#elif defined                                                                  \
    SAFEINT_FAILFAST // Call __failfast or abort, depending on platform
...
public:
  _CONSTEXPR11
  SafeIntException(SafeIntError code = SafeIntNoError) SAFEINT_NOTHROW
      : m_code(code) {}
  SafeIntError m_code;
};
...
#define SafeIntDefaultExceptionHandler                                         \
  safeint_exception_handlers::CPlusPlusExceptionHandler
#define SAFEINT_EXCEPTION_HANDLER_CPP 1
...
public:
  static SAFEINT_NORETURN void SAFEINT_STDCALL SafeIntOnOverflow()
      SAFEINT_NOTHROW {
    SafeIntExceptionAssert();
    RaiseException(static_cast<DWORD>(EXCEPTION_INT_OVERFLOW),
  ...
  static SAFEINT_NORETURN void SAFEINT_STDCALL SafeIntOnDivZero()
      SAFEINT_NOTHROW {
    SafeIntExceptionAssert();
    RaiseException(static_cast<DWORD>(EXCEPTION_INT_DIVIDE_BY_ZERO),
                   EXCEPTION_NONCONTINUABLE, 0, 0);
  }
};
...
typedef safeint_exception_handlers::SafeIntWin32ExceptionHandler
...
typedef safeint_exception_handlers::SafeInt_InvalidParameter
    InvalidParameterExceptionHandler;
#define SafeIntDefaultExceptionHandler InvalidParameterExceptionHandler
#define SAFEINT_EXCEPTION_HANDLER_CPP 0
#endif

// If an error handler is chosen other than C++ exceptions, such as Win32
// exceptions, fail fast, or abort, then all methods become no throw. Some teams
// track throw() annotations closely, and the following option provides for
// this.
...
// If we have support for std<typetraits>, then we can do this easily, and
// detect enums as well
template <typename T> class numeric_type;

// Continue to special case bool
template <> class numeric_type<bool> {
public:
  enum { isBool = true, isInt = false };
};
template <typename T> class numeric_type {
public:
  enum {
    ...
    // This does allow someone to make a SafeInt from an enum type, which is not
    // recommended, but it also allows someone to add an enum value to a
    // SafeInt, which is handy.
...
public:
  static_assert(safeint_internal::numeric_type<T>::isInt,
...
public:
  enum {
    isBothSigned = (std::numeric_limits<T>::is_signed &&
                    std::numeric_limits<U>::is_signed),
    isBothUnsigned = (!std::numeric_limits<T>::is_signed &&
                      !std::numeric_limits<U>::is_signed),
    ...
    isBothLT32Bit = (safeint_internal::int_traits<T>::isLT32Bit &&
                     safeint_internal::int_traits<U>::isLT32Bit),
    isBothLT64Bit = (safeint_internal::int_traits<T>::isLT64Bit &&
                     safeint_internal::int_traits<U>::isLT64Bit)
  };
};
...
// most operators collapse the problem into less than the 22 zones, but this is
// used as the first cut using this also helps ensure that we handle all of the
...
public:
  enum {
    ...
    IntZone_UintLT32_UintLT32 =
        ...
        safeint_internal::type_compare<T, U>::isBothLT32Bit,
    IntZone_Uint32_UintLT64 =
        safeint_internal::type_compare<T, U>::isBothUnsigned &&
        safeint_internal::int_traits<T>::is32Bit &&
        safeint_internal::int_traits<U>::isLT64Bit,
    IntZone_UintLT32_Uint32 =
        safeint_internal::type_compare<T, U>::isBothUnsigned &&
        safeint_internal::int_traits<T>::isLT32Bit &&
        safeint_internal::int_traits<U>::is32Bit,
    IntZone_Uint64_Uint =
        safeint_internal::type_compare<T, U>::isBothUnsigned &&
        safeint_internal::int_traits<T>::is64Bit,
    IntZone_UintLT64_Uint64 =
        safeint_internal::type_compare<T, U>::isBothUnsigned &&
        safeint_internal::int_traits<T>::isLT64Bit &&
        safeint_internal::int_traits<U>::is64Bit,
    // unsigned-signed
    IntZone_UintLT32_IntLT32 =
        !std::numeric_limits<T>::is_signed &&
        std::numeric_limits<U>::is_signed &&
        safeint_internal::type_compare<T, U>::isBothLT32Bit,
    IntZone_Uint32_IntLT64 = safeint_internal::int_traits<T>::isUint32 &&
                             std::numeric_limits<U>::is_signed &&
                             safeint_internal::int_traits<U>::isLT64Bit,
    IntZone_UintLT32_Int32 = !std::numeric_limits<T>::is_signed &&
                             safeint_internal::int_traits<T>::isLT32Bit &&
                             safeint_internal::int_traits<U>::isInt32,
    IntZone_Uint64_Int = safeint_internal::int_traits<T>::isUint64 &&
                         std::numeric_limits<U>::is_signed &&
                         safeint_internal::int_traits<U>::isLT64Bit,
    IntZone_UintLT64_Int64 = !std::numeric_limits<T>::is_signed &&
                             safeint_internal::int_traits<T>::isLT64Bit &&
                             safeint_internal::int_traits<U>::isInt64,
    IntZone_Uint64_Int64 = safeint_internal::int_traits<T>::isUint64 &&
                           safeint_internal::int_traits<U>::isInt64,
    // signed-signed
    IntZone_IntLT32_IntLT32 =
        safeint_internal::type_compare<T, U>::isBothSigned &&
        safeint_internal::type_compare<T, U>::isBothLT32Bit,
    IntZone_Int32_IntLT64 =
        safeint_internal::type_compare<T, U>::isBothSigned &&
        safeint_internal::int_traits<T>::is32Bit &&
        safeint_internal::int_traits<U>::isLT64Bit,
    IntZone_IntLT32_Int32 =
        safeint_internal::type_compare<T, U>::isBothSigned &&
        safeint_internal::int_traits<T>::isLT32Bit &&
        safeint_internal::int_traits<U>::is32Bit,
    IntZone_Int64_Int64 = safeint_internal::type_compare<T, U>::isBothSigned &&
                          safeint_internal::int_traits<T>::isInt64 &&
                          safeint_internal::int_traits<U>::isInt64,
    IntZone_Int64_Int = safeint_internal::type_compare<T, U>::isBothSigned &&
                        safeint_internal::int_traits<T>::is64Bit &&
                        safeint_internal::int_traits<U>::isLT64Bit,
    ...
    IntZone_IntLT32_UintLT32 =
        ...
        !std::numeric_limits<U>::is_signed &&
        safeint_internal::type_compare<T, U>::isBothLT32Bit,
    IntZone_Int32_UintLT32 = safeint_internal::int_traits<T>::isInt32 &&
                             !std::numeric_limits<U>::is_signed &&
                             safeint_internal::int_traits<U>::isLT32Bit,
    IntZone_IntLT64_Uint32 = std::numeric_limits<T>::is_signed &&
                             safeint_internal::int_traits<T>::isLT64Bit &&
                             safeint_internal::int_traits<U>::isUint32,
    IntZone_Int64_UintLT64 = safeint_internal::int_traits<T>::isInt64 &&
                             !std::numeric_limits<U>::is_signed &&
                             safeint_internal::int_traits<U>::isLT64Bit,
    IntZone_Int_Uint64 = std::numeric_limits<T>::is_signed &&
                         safeint_internal::int_traits<U>::isUint64 &&
                         safeint_internal::int_traits<T>::isLT64Bit,
    IntZone_Int64_Uint64 = safeint_internal::int_traits<T>::isInt64 &&
                           safeint_internal::int_traits<U>::isUint64
  };
};
...
// The non-throwing versions are for use by the helper functions that return
// success and failure. Some of the non-throwing functions are not used, but are
// maintained for completeness.

// There's no real alternative to duplicating logic, but keeping the two
// versions immediately next to one another will help reduce problems
...
template <typename T> class GetAbsMethod {
public:
  enum {
    method = safeint_internal::int_traits<T>::isLT64Bit &&
                     std::numeric_limits<T>::is_signed
                 ? AbsMethodInt
             : safeint_internal::int_traits<T>::isInt64 ? AbsMethodInt64
                                                        : AbsMethodNoop
  };
};
...
public:
  SAFE_INT_NODISCARD _CONSTEXPR14 static std::uint32_t
  Abs(T t) SAFEINT_NOTHROW {
    SAFEINT_ASSERT(t < 0);
...
public:
  SAFE_INT_NODISCARD _CONSTEXPR14 static std::uint64_t
  Abs(T t) SAFEINT_NOTHROW {
    SAFEINT_ASSERT(t < 0);
...
template <> class SignedNegation<std::int32_t> {
public:
...
template <> class SignedNegation<std::int64_t> {
public:
...
// Previous versions had an assert that the type being negated was 32-bit or
// higher In retrospect, this seems like something to just document Negation
// will normally upcast to int For example -(unsigned short)0xffff ==
// (int)0xffff0001 This class will retain the type, and will truncate, which may
// not be what you wanted If you want normal operator casting behavior, do this:
...
public:
  template <typename E>
  SAFE_INT_NODISCARD _CONSTEXPR14 static T
  NegativeThrow(T t) SAFEINT_CPP_THROW {
    // corner case
...
#endif
    // This may not be the most efficient approach, but you shouldn't be doing
    // this
...
template <typename ToType, typename FromType> class GetCastMethod {
public:
  enum {
    method =
        (safeint_internal::numeric_type<FromType>::isEnum) ? CastFromEnum
        : (safeint_internal::int_traits<FromType>::isBool &&
           !safeint_internal::int_traits<ToType>::isBool)
            ? CastFromBool
        :
...
template <typename T>
class float_cast_helper<T, true> // Unsigned case
{
public:
  SAFE_INT_NODISCARD _CONSTEXPR14 static bool Test(double d) {
    const std::uint64_t signifDouble = 0x1fffffffffffff;

    // Anything larger than this either is larger than 2^64-1, or cannot be
...
template <typename T>
class float_cast_helper<T, false> // Signed case
{
public:
  SAFE_INT_NODISCARD _CONSTEXPR14 static bool Test(double d) {
    const std::uint64_t signifDouble = 0x1fffffffffffff;
    ...
    const std::int64_t minSignedDouble =
        static_cast<std::int64_t>(0x8000000000000000);

    if (d < static_cast<double>(minSignedDouble) ||
        d > static_cast<double>(maxSignedDouble))
      return false;

    // And now cast to long long, and check against min and max for this type
    std::int64_t test = static_cast<std::int64_t>(d);
    if ((std::int64_t)test < (std::int64_t)std::numeric_limits<T>::min() ||
...
public:
  ...
  template <typename E> static void CastThrow(U u, T &t) SAFEINT_CPP_THROW {
    if (CheckFloatingPointCast(u)) {
      t = (T)u;
      return;
    }
...
public:
  SAFE_INT_NODISCARD _CONSTEXPR14 static bool Cast(U u, T &t) SAFEINT_NOTHROW {
    return SafeCastHelper<T, int, GetCastMethod<T, int>::method>::Cast(
        static_cast<int>(u), t);
  }

  template <typename E>
  _CONSTEXPR14 static void CastThrow(U u, T &t) SAFEINT_CPP_THROW {
    SafeCastHelper<T, int, GetCastMethod<T, int>::method>::template CastThrow<
        E>(static_cast<int>(u), t);
  }
};
...
public:
  SAFE_INT_NODISCARD _CONSTEXPR14 static bool Cast(bool b,
                                                   T &t) SAFEINT_NOTHROW {
    t = (T)(b ? 1 : 0);
    return true;
  }
...
public:
  SAFE_INT_NODISCARD _CONSTEXPR14 static bool Cast(T t,
                                                   bool &b) SAFEINT_NOTHROW {
    b = !!t;
    return true;
  }

  // CastThrow is not needed.
  // This is because CastThrow is only ever called inside various multiply,
  // divide, add, and subtract methods, none of which make sense for one of the
  // operands to be bool. It is also called within the various direct casts that
  // will cast the SafeInt instance to various types, but there is nothing to
  // check for a cast to bool, and it is implemented directly without needing
  // this method.

  // There is a call to Cast because it is used in the SafeCast non-throwing
  // function.
};
...
// core logic to determine whether a comparison is valid, or needs special
// treatment
...
// a cast to an std::int64_t, which can clearly preserve both value and
// signedness then you get a different and intuitively correct answer IMHO, -1
// should be less than 4 billion If you prefer to retain the ANSI standard
// behavior insert #define ANSI_CONVERSIONS into your source Behavior
// differences occur in the following cases: 8, 16, and 32-bit signed int,
// unsigned 32-bit int any signed int, unsigned 64-bit int Note - the signed int
// must be negative to show the problem

template <typename T, typename U> class ValidComparison {
public:
  enum {
    method =
        ((safeint_internal::type_compare<T, U>::isLikeSigned)
             ? ComparisonMethod_Ok
         : ((std::numeric_limits<T>::is_signed && sizeof(T) < 8 &&
             sizeof(U) < 4) ||
            (std::numeric_limits<U>::is_signed && sizeof(T) < 4 &&
             sizeof(U) < 8))
             ? ComparisonMethod_CastInt
...
public:
  SAFE_INT_NODISCARD _CONSTEXPR11 static bool
  IsEquals(const T t, const U u) SAFEINT_NOTHROW {
    return (t == u);
  }
};
...
public:
  SAFE_INT_NODISCARD _CONSTEXPR11 static bool
  IsEquals(const T t, const U u) SAFEINT_NOTHROW {
    return ((std::int64_t)t == (std::int64_t)u);
  }
};
...
public:
  SAFE_INT_NODISCARD _CONSTEXPR14 static bool
  IsEquals(const T t, const U u) SAFEINT_NOTHROW {
    // one operand is 32 or 64-bit unsigned, and the other is signed and the
...
public:
  SAFE_INT_NODISCARD _CONSTEXPR11 static bool
  GreaterThan(const T t, const U u) SAFEINT_NOTHROW {
    return (t > u);
  }
};
...
template <typename U>
class mod_corner_case<U, true> // signed
{
public:
...
template <typename U>
class mod_corner_case<U, false> // unsigned
{
public:
  SAFE_INT_NODISCARD _CONSTEXPR14 static bool is_undefined(U) { return false; }
};

template <typename T, typename U>
class ModulusHelper<T, U, ComparisonMethod_Ok> {
public:
  SAFE_INT_NODISCARD _CONSTEXPR14 static SafeIntError
  Modulus(const T &t, const U &u, T &result) SAFEINT_NOTHROW {
    if (u == 0)
      return SafeIntDivideByZero;

    // trap corner case
    if (mod_corner_case<U, std::numeric_limits<U>::is_signed>::is_undefined(
            u)) {
      result = 0;
  ...
  template <typename E>
  _CONSTEXPR14 static void ModulusThrow(const T &t, const U &u,
                                        T &result) SAFEINT_CPP_THROW {
    if (u == 0)
      E::SafeIntOnDivZero();
...
public:
  ...
  Modulus(const T &t, const U &u, T &result) SAFEINT_NOTHROW {
    ...
    if (t < 0)
      result =
          (T)(~(AbsValueHelper<T, GetAbsMethod<T>::method>::Abs(t) % u) + 1);
    else
      result = (T)((T)t % u);
...
enum MultiplicationState {
  ...
  MultiplicationState_Uint64Uint,   // Both are unsigned, lhs 64-bit, rhs 32-bit
                                    // or smaller
  ...
  MultiplicationState_UintUint64,   // Both are unsigned, lhs 32-bit or smaller,
                                    // rhs 64-bit
...
template <typename T, typename U> class MultiplicationMethod {
public:
  enum {
    // unsigned-unsigned
    method =
        (IntRegion<T, U>::IntZone_UintLT32_UintLT32
         ...
         : safeint_internal::type_compare<T, U>::isBothUnsigned &&
                 safeint_internal::int_traits<T>::isUint64 &&
                 safeint_internal::int_traits<U>::isUint64
         ...
         : (IntRegion<T, U>::IntZone_Int_Uint64)
             ...
             : (IntRegion<T, U>::IntZone_Int64_Uint64
...
public:
  ...
  SAFE_INT_NODISCARD _CONSTEXPR14 static bool Multiply(const T &t, const U &u,
                                                       T &ret) SAFEINT_NOTHROW {
    int tmp = t * u;
  ...
  template <typename E>
  _CONSTEXPR14 static void MultiplyThrow(const T &t, const U &u,
                                         T &ret) SAFEINT_CPP_THROW {
    int tmp = t * u;
...
public:
  // mixed signed or both signed where at least one argument is 32-bit, and both
  // a 32-bit or less
  SAFE_INT_NODISCARD _CONSTEXPR14 static bool Multiply(const T &t, const U &u,
                                                       T &ret) SAFEINT_NOTHROW {
    std::int64_t tmp = (std::int64_t)t * (std::int64_t)u;

    if (tmp > (std::int64_t)std::numeric_limits<T>::max() ||
        tmp < (std::int64_t)std::numeric_limits<T>::min())
      return false;
...
public:
  // both unsigned where at least one argument is 32-bit, and both are 32-bit or
  // less
  SAFE_INT_NODISCARD _CONSTEXPR14 static bool Multiply(const T &t, const U &u,
                                                       T &ret) SAFEINT_NOTHROW {
...
_CONSTEXPR14 inline bool MultiplyUint64(std::uint64_t a, std::uint64_t b,
                                        std::uint64_t *pRet) SAFEINT_NOTHROW {
...
_CONSTEXPR14 inline bool MultiplyInt64(std::int64_t a, std::int64_t b,
                                       std::int64_t *pRet) SAFEINT_NOTHROW {
  ...
  *pRet = (std::int64_t)tmp;
  ...
  if ((a ^ b) < 0) {
    if ((tmp_high == -1 && *pRet < 0) || (tmp_high == 0 && *pRet == 0)) {
      return true;
    }
  } else {
    if (tmp_high == 0) {
      return (std::uint64_t)*pRet <=
...
inline bool MultiplyUint64(std::uint64_t a, std::uint64_t b,
                           std::uint64_t *pRet) SAFEINT_NOTHROW {
...
inline bool MultiplyInt64(std::int64_t a, std::int64_t b,
                          std::int64_t *pRet) SAFEINT_NOTHROW {
...
public:
  SAFE_INT_NODISCARD _CONSTEXPR14_MULTIPLY static bool
  RegMultiply(const std::uint64_t &a, const std::uint64_t &b,
              std::uint64_t *pRet) SAFEINT_NOTHROW {
    ...
    // => (aHigh * bHigh * 2^64) + (aLow * bHigh * 2^32) + (aHigh * bLow * 2^32)
    // + (aLow * bLow) Note - same approach applies for 128 bit math on a 64-bit
    // system
  ...
  template <typename E>
  _CONSTEXPR14_MULTIPLY static void
  RegMultiplyThrow(const std::uint64_t &a, const std::uint64_t &b,
                   std::uint64_t *pRet) SAFEINT_CPP_THROW {
#if SAFEINT_USE_INTRINSICS || SAFEINT_HAS_INT128
...
public:
  SAFE_INT_NODISCARD _CONSTEXPR14_MULTIPLY static bool
  RegMultiply(const std::uint64_t &a, std::uint32_t b,
              std::uint64_t *pRet) SAFEINT_NOTHROW {
#if SAFEINT_USE_INTRINSICS || SAFEINT_HAS_INT128
  ...
  template <typename E>
  _CONSTEXPR14_MULTIPLY static void
  RegMultiplyThrow(const std::uint64_t &a, std::uint32_t b,
                   std::uint64_t *pRet) SAFEINT_CPP_THROW {
#if SAFEINT_USE_INTRINSICS || SAFEINT_HAS_INT128
...
public:
  ...
  SAFE_INT_NODISCARD _CONSTEXPR14_MULTIPLY static bool
  RegMultiply(const std::uint64_t &a, std::int32_t b,
              std::uint64_t *pRet) SAFEINT_NOTHROW {
    if (b < 0 && a != 0)
      return false;
    ...
    return MultiplyUint64(a, (std::uint64_t)b, pRet);
#else
    return LargeIntRegMultiply<std::uint64_t, std::uint32_t>::RegMultiply(
        a, (std::uint32_t)b, pRet);
#endif
  }

  template <typename E>
  _CONSTEXPR14_MULTIPLY static void
  RegMultiplyThrow(const std::uint64_t &a, std::int32_t b,
                   std::uint64_t *pRet) SAFEINT_CPP_THROW {
    if (b < 0 && a != 0)
      E::SafeIntOnOverflow();
    ...
    if (!MultiplyUint64(a, (std::uint64_t)b, pRet))
      E::SafeIntOnOverflow();
#else
    LargeIntRegMultiply<std::uint64_t, std::uint32_t>::
        template RegMultiplyThrow<E>(a, (std::uint32_t)b, pRet);
#endif
  }
};
...
public:
  SAFE_INT_NODISCARD _CONSTEXPR14_MULTIPLY static bool
  RegMultiply(const std::uint64_t &a, std::int64_t b,
              std::uint64_t *pRet) SAFEINT_NOTHROW {
    if (b < 0 && a != 0)
      return false;
    ...
    return MultiplyUint64(a, (std::uint64_t)b, pRet);
#else
    return LargeIntRegMultiply<std::uint64_t, std::uint64_t>::RegMultiply(
        a, (std::uint64_t)b, pRet);
#endif
  }

  template <typename E>
  _CONSTEXPR14_MULTIPLY static void
  RegMultiplyThrow(const std::uint64_t &a, std::int64_t b,
                   std::uint64_t *pRet) SAFEINT_CPP_THROW {
    if (b < 0 && a != 0)
      E::SafeIntOnOverflow();
    ...
    if (!MultiplyUint64(a, (std::uint64_t)b, pRet))
      E::SafeIntOnOverflow();
#else
    LargeIntRegMultiply<std::uint64_t, std::uint64_t>::
        template RegMultiplyThrow<E>(a, (std::uint64_t)b, pRet);
#endif
  }
};
...
public:
  ...
  SAFE_INT_NODISCARD _CONSTEXPR14 static bool
  RegMultiply(std::int32_t a, const std::uint64_t &b,
              std::int32_t *pRet) SAFEINT_NOTHROW {
    ...
    // (aHigh * 2^32 + aLow) * (bHigh * 2^32 + bLow)
    // => (aHigh * bHigh * 2^64) + (aLow * bHigh * 2^32) + (aHigh * bLow * 2^32)
    // + (aLow * bLow) aHigh == 0 implies: ( aLow * bHigh * 2^32 ) + ( aLow +
    // bLow ) If the first part is != 0, fail
    ...
    *pRet = 0;
    ...
    if (a < 0) {
      ...
      a = (std::int32_t)
          AbsValueHelper<std::int32_t, GetAbsMethod<std::int32_t>::method>::Abs(
              a);
      fIsNegative = true;
    }
  ...
  template <typename E>
  _CONSTEXPR14 static void
  RegMultiplyThrow(std::int32_t a, const std::uint64_t &b,
                   std::int32_t *pRet) SAFEINT_CPP_THROW {
    std::uint32_t bHigh = 0, bLow = 0;
    bool fIsNegative = false;
...
public:
  ...
  SAFE_INT_NODISCARD _CONSTEXPR14 static bool
  RegMultiply(std::uint32_t a, const std::uint64_t &b,
              std::uint32_t *pRet) SAFEINT_NOTHROW {
    // Consider that a*b can be broken up into:
    // (bHigh * 2^32 + bLow) * a
    // => (bHigh * a * 2^32) + (bLow * a)
  ...
  template <typename E>
  _CONSTEXPR14 static void
  RegMultiplyThrow(std::uint32_t a, const std::uint64_t &b,
                   std::uint32_t *pRet) SAFEINT_CPP_THROW {
    if ((std::uint32_t)(b >> 32) != 0 && a != 0)
      E::SafeIntOnOverflow();
...
public:
  SAFE_INT_NODISCARD _CONSTEXPR14 static bool
  RegMultiply(std::uint32_t a, const std::int64_t &b,
              std::uint32_t *pRet) SAFEINT_NOTHROW {
    if (b < 0 && a != 0)
      return false;
    return LargeIntRegMultiply<std::uint32_t, std::uint64_t>::RegMultiply(
        a, (std::uint64_t)b, pRet);
  }

  template <typename E>
  _CONSTEXPR14 static void
  RegMultiplyThrow(std::uint32_t a, const std::int64_t &b,
                   std::uint32_t *pRet) SAFEINT_CPP_THROW {
    if (b < 0 && a != 0)
      E::SafeIntOnOverflow();

    LargeIntRegMultiply<std::uint32_t, std::uint64_t>::
        template RegMultiplyThrow<E>(a, (std::uint64_t)b, pRet);
  }
};
...
public:
  SAFE_INT_NODISCARD _CONSTEXPR14_MULTIPLY static bool
  RegMultiply(const std::int64_t &a, const std::int64_t &b,
              std::int64_t *pRet) SAFEINT_NOTHROW {
#if SAFEINT_USE_INTRINSICS || SAFEINT_HAS_INT128
    ...
    if (a1 < 0) {
      aNegative = true;
      a1 = (std::int64_t)
          AbsValueHelper<std::int64_t, GetAbsMethod<std::int64_t>::method>::Abs(
              a1);
    }
  ...
  template <typename E>
  _CONSTEXPR14_MULTIPLY static void
  RegMultiplyThrow(const std::int64_t &a, const std::int64_t &b,
                   std::int64_t *pRet) SAFEINT_CPP_THROW {
#if SAFEINT_USE_INTRINSICS || SAFEINT_HAS_INT128
    ...
    LargeIntRegMultiply<std::uint64_t, std::uint64_t>::
        template RegMultiplyThrow<E>((std::uint64_t)a1, (std::uint64_t)b1,
                                     &tmp);

    // The unsigned multiplication didn't overflow or we'd be in the exception
    // handler
    if (aNegative ^ bNegative) {
      // Result must be negative
      if (tmp <= (std::uint64_t)std::numeric_limits<std::int64_t>::min()) {
...
public:
  SAFE_INT_NODISCARD _CONSTEXPR14_MULTIPLY static bool
  RegMultiply(const std::int64_t &a, std::uint32_t b,
              std::int64_t *pRet) SAFEINT_NOTHROW {
#if SAFEINT_USE_INTRINSICS || SAFEINT_HAS_INT128
  ...
  template <typename E>
  _CONSTEXPR14_MULTIPLY static void
  RegMultiplyThrow(const std::int64_t &a, std::uint32_t b,
                   std::int64_t *pRet) SAFEINT_CPP_THROW {
#if SAFEINT_USE_INTRINSICS || SAFEINT_HAS_INT128
    ...
    LargeIntRegMultiply<std::uint64_t, std::uint32_t>::
        template RegMultiplyThrow<E>((std::uint64_t)a1, b, &tmp);
...
public:
  SAFE_INT_NODISCARD _CONSTEXPR14_MULTIPLY static bool
  RegMultiply(const std::int64_t &a, std::int32_t b,
              std::int64_t *pRet) SAFEINT_NOTHROW {
#if SAFEINT_USE_INTRINSICS || SAFEINT_HAS_INT128
    return MultiplyInt64(a, (std::int64_t)b, pRet);
#else
  ...
  template <typename E>
  _CONSTEXPR14_MULTIPLY static void
  RegMultiplyThrow(std::int64_t a, std::int32_t b,
                   std::int64_t *pRet) SAFEINT_CPP_THROW {
#if SAFEINT_USE_INTRINSICS || SAFEINT_HAS_INT128
    if (!MultiplyInt64(a, (std::int64_t)b, pRet))
      E::SafeIntOnOverflow();
    ...
    LargeIntRegMultiply<std::uint64_t, std::uint32_t>::
        template RegMultiplyThrow<E>((std::uint64_t)a, (std::uint32_t)b, &tmp);
...
public:
  SAFE_INT_NODISCARD _CONSTEXPR14_MULTIPLY static bool
  RegMultiply(std::int32_t a, const std::int64_t &b,
              std::int32_t *pRet) SAFEINT_NOTHROW {
#if SAFEINT_USE_INTRINSICS || SAFEINT_HAS_INT128
    std::int64_t tmp = 0;
  ...
  template <typename E>
  _CONSTEXPR14_MULTIPLY static void
  RegMultiplyThrow(std::int32_t a, const std::int64_t &b,
                   std::int32_t *pRet) SAFEINT_CPP_THROW {
#if SAFEINT_USE_INTRINSICS || SAFEINT_HAS_INT128
    std::int64_t tmp = 0;
    ...
    LargeIntRegMultiply<std::uint32_t, std::uint64_t>::
        template RegMultiplyThrow<E>((std::uint32_t)a, (std::uint64_t)b2, &tmp);
...
public:
  ...
  SAFE_INT_NODISCARD _CONSTEXPR14_MULTIPLY static bool
  RegMultiply(const std::int64_t &a, const std::uint64_t &b,
              std::int64_t *pRet) SAFEINT_NOTHROW {
    bool aNegative = false;
  ...
  template <typename E>
  _CONSTEXPR14_MULTIPLY static void
  RegMultiplyThrow(const std::int64_t &a, const std::uint64_t &b,
                   std::int64_t *pRet) SAFEINT_CPP_THROW {
    bool aNegative = false;
    std::uint64_t tmp = 0;
    std::int64_t a1 = a;
...
// In all of the following functions where LargeIntRegMultiply methods are
// called, we need to properly transition types. The methods need std::int64_t,
// std::int32_t, etc. but the variables being passed to us could be long long,
// long int, or long, depending on the compiler. Microsoft compiler knows that
// long long is the same type as std::int64_t, but gcc doesn't
...
public:
  ...
  SAFE_INT_NODISCARD _CONSTEXPR14_MULTIPLY static bool
  Multiply(const T &t, const U &u, T &ret) SAFEINT_NOTHROW {
    static_assert(safeint_internal::int_traits<T>::isUint64 &&
                      safeint_internal::int_traits<U>::isUint64,
                  "T, U must be Uint64");
    std::uint64_t t1 = t;
    std::uint64_t u1 = u;
    std::uint64_t tmp = 0;
    bool f = LargeIntRegMultiply<std::uint64_t, std::uint64_t>::RegMultiply(
        t1, u1, &tmp);
    ret = tmp;
    return f;
  }
  ...
  template <typename E>
  _CONSTEXPR14_MULTIPLY static void MultiplyThrow(const std::uint64_t &t,
    ...
    std::uint64_t u1 = u;
    std::uint64_t tmp = 0;
    LargeIntRegMultiply<std::uint64_t,
                        std::uint64_t>::template RegMultiplyThrow<E>(t1, u1,
                                                                     &tmp);
    ret = tmp;
  }
};
...
public:
  ...
  Multiply(const T &t, const U &u, T &ret) SAFEINT_NOTHROW {
    ...
    std::uint64_t t1 = t;
    std::uint64_t tmp = 0;
    bool f = LargeIntRegMultiply<std::uint64_t, std::uint32_t>::RegMultiply(
        t1, (std::uint32_t)u, &tmp);
    ret = tmp;
    return f;
  }

  template <typename E>
  _CONSTEXPR14_MULTIPLY static void MultiplyThrow(const T &t, const U &u,
                                                  T &ret) SAFEINT_CPP_THROW {
    static_assert(safeint_internal::int_traits<T>::isUint64,
                  "T must be Uint64");
    std::uint64_t t1 = t;
    std::uint64_t tmp = 0;
    LargeIntRegMultiply<std::uint64_t, std::uint32_t>::
        template RegMultiplyThrow<E>(t1, (std::uint32_t)u, &tmp);
    ret = tmp;
  }
};
...
public:
  ...
  SAFE_INT_NODISCARD _CONSTEXPR14 static bool Multiply(const T &t, const U &u,
                                                       T &ret) SAFEINT_NOTHROW {
    static_assert(safeint_internal::int_traits<U>::isUint64,
                  "U must be Uint64");
    std::uint64_t u1 = u;
    std::uint32_t tmp = 0;

    if (LargeIntRegMultiply<std::uint32_t, std::uint64_t>::RegMultiply(t, u1,
                                                                       &tmp) &&
        SafeCastHelper<T, std::uint32_t,
                       GetCastMethod<T, std::uint32_t>::method>::Cast(tmp,
                                                                      ret)) {
      return true;
    }
  ...
  _CONSTEXPR14 static void MultiplyThrow(const T &t, const U &u,
    ...
    LargeIntRegMultiply<std::uint32_t,
                        std::uint64_t>::template RegMultiplyThrow<E>(t, u1,
                                                                     &tmp);
    SafeCastHelper<T, std::uint32_t, GetCastMethod<T, std::uint32_t>::method>::
        template CastThrow<E>(tmp, ret);
  }
};
...
public:
  ...
  Multiply(const T &t, const U &u, T &ret) SAFEINT_NOTHROW {
    ...
    std::uint64_t t1 = t;
    std::uint64_t tmp = 0;
    bool f = LargeIntRegMultiply<std::uint64_t, std::int32_t>::RegMultiply(
        t1, (std::int32_t)u, &tmp);
    ret = tmp;
    return f;
  }
  ...
  _CONSTEXPR14_MULTIPLY static void MultiplyThrow(const T &t, const U &u,
    ...
    std::uint64_t t1 = t;
    std::uint64_t tmp = 0;
    LargeIntRegMultiply<std::uint64_t, std::int32_t>::template RegMultiplyThrow<
        E>(t1, (std::int32_t)u, &tmp);
    ret = tmp;
  }
};
...
public:
  ...
  Multiply(const T &t, const U &u, T &ret) SAFEINT_NOTHROW {
    ...
    std::int64_t u1 = u;
    std::uint64_t tmp = 0;
    bool f = LargeIntRegMultiply<std::uint64_t, std::int64_t>::RegMultiply(
        t1, u1, &tmp);
    ret = tmp;
    return f;
  }
  ...
  _CONSTEXPR14_MULTIPLY static void MultiplyThrow(const T &t, const U &u,
    ...
    std::uint64_t tmp = 0;
    LargeIntRegMultiply<std::uint64_t,
                        std::int64_t>::template RegMultiplyThrow<E>(t1, u1,
                                                                    &tmp);
    ret = tmp;
  }
};
...
public:
  ...
  SAFE_INT_NODISCARD _CONSTEXPR14 static bool Multiply(const T &t, const U &u,
    ...
    if (LargeIntRegMultiply<std::uint32_t, std::int64_t>::RegMultiply(
            (std::uint32_t)t, u1, &tmp) &&
        SafeCastHelper<T, std::uint32_t,
                       GetCastMethod<T, std::uint32_t>::method>::Cast(tmp,
                                                                      ret)) {
      return true;
  ...
  _CONSTEXPR14 static void MultiplyThrow(const T &t, const U &u,
    ...
    LargeIntRegMultiply<std::uint32_t, std::int64_t>::template RegMultiplyThrow<
        E>((std::uint32_t)t, u1, &tmp);
    SafeCastHelper<T, std::uint32_t, GetCastMethod<T, std::uint32_t>::method>::
        template CastThrow<E>(tmp, ret);
  }
};
...
public:
  ...
  Multiply(const T &t, const U &u, T &ret) SAFEINT_NOTHROW {
    ...
    std::int64_t t1 = t;
    std::int64_t tmp = 0;
    bool f = LargeIntRegMultiply<std::int64_t, std::uint32_t>::RegMultiply(
        t1, (std::uint32_t)u, &tmp);
    ret = tmp;
    return f;
  }
  ...
  _CONSTEXPR14_MULTIPLY static void MultiplyThrow(const T &t, const U &u,
    ...
    std::int64_t t1 = t;
    std::int64_t tmp = 0;
    LargeIntRegMultiply<std::int64_t, std::uint32_t>::template RegMultiplyThrow<
        E>(t1, (std::uint32_t)u, &tmp);
    ret = tmp;
  }
};
...
public:
  ...
  SAFE_INT_NODISCARD _CONSTEXPR14_MULTIPLY static bool
  Multiply(const T &t, const U &u, T &ret) SAFEINT_NOTHROW {
    static_assert(safeint_internal::int_traits<T>::isInt64 &&
                      safeint_internal::int_traits<U>::isInt64,
                  "T, U must be Int64");
    std::int64_t t1 = t;
    std::int64_t u1 = u;
    std::int64_t tmp = 0;
    bool f = LargeIntRegMultiply<std::int64_t, std::int64_t>::RegMultiply(
        t1, u1, &tmp);
    ret = tmp;
    return f;
  }
  ...
  _CONSTEXPR14_MULTIPLY static void MultiplyThrow(const T &t, const U &u,
    ...
    std::int64_t u1 = u;
    std::int64_t tmp = 0;
    LargeIntRegMultiply<std::int64_t,
                        std::int64_t>::template RegMultiplyThrow<E>(t1, u1,
                                                                    &tmp);
    ret = tmp;
  }
...
public:
  ...
  SAFE_INT_NODISCARD _CONSTEXPR14_MULTIPLY static bool
  Multiply(const T &t, U u, T &ret) SAFEINT_NOTHROW {
    static_assert(safeint_internal::int_traits<T>::isInt64, "T must be Int64");
    std::int64_t t1 = t;
    std::int64_t tmp = 0;
    bool f = LargeIntRegMultiply<std::int64_t, std::int32_t>::RegMultiply(
        t1, (std::int32_t)u, &tmp);
    ret = tmp;
    return f;
  }
  ...
  template <typename E>
  _CONSTEXPR14_MULTIPLY static void MultiplyThrow(const std::int64_t &t, U u,
                                                  T &ret) SAFEINT_CPP_THROW {
    static_assert(safeint_internal::int_traits<T>::isInt64, "T must be Int64");
    std::int64_t t1 = t;
    std::int64_t tmp = 0;
    LargeIntRegMultiply<std::int64_t, std::int32_t>::template RegMultiplyThrow<
        E>(t1, (std::int32_t)u, &tmp);
    ret = tmp;
  }
};
...
public:
  ...
  // U is std::uint64_t
  SAFE_INT_NODISCARD _CONSTEXPR14 static bool Multiply(T t, const U &u,
                                                       T &ret) SAFEINT_NOTHROW {
    static_assert(safeint_internal::int_traits<U>::isUint64,
                  "U must be Uint64");
    std::uint64_t u1 = u;
    ...
    if (LargeIntRegMultiply<std::int32_t, std::uint64_t>::RegMultiply(
            (std::int32_t)t, u1, &tmp) &&
        SafeCastHelper<T, std::int32_t,
                       GetCastMethod<T, std::int32_t>::method>::Cast(tmp,
                                                                     ret)) {
      return true;
    }
  ...
  template <typename E>
  _CONSTEXPR14 static void MultiplyThrow(T t, const std::uint64_t &u,
                                         T &ret) SAFEINT_CPP_THROW {
    static_assert(safeint_internal::int_traits<U>::isUint64,
                  "U must be Uint64");
    std::uint64_t u1 = u;
    ...
    LargeIntRegMultiply<std::int32_t, std::uint64_t>::template RegMultiplyThrow<
        E>((std::int32_t)t, u1, &tmp);
    SafeCastHelper<T, std::int32_t, GetCastMethod<T, std::int32_t>::method>::
        template CastThrow<E>(tmp, ret);
  }
};
...
public:
  ...
  Multiply(const T &t, const U &u, T &ret) SAFEINT_NOTHROW {
    ...
    std::uint64_t u1 = u;
    std::int64_t tmp = 0;
    bool f = LargeIntRegMultiply<std::int64_t, std::uint64_t>::RegMultiply(
        t1, u1, &tmp);
    ret = tmp;
    return f;
  }
  ...
  template <typename E>
  _CONSTEXPR14_MULTIPLY static void MultiplyThrow(const std::int64_t &t,
                                                  const std::uint64_t &u,
                                                  T &ret) SAFEINT_CPP_THROW {
    static_assert(safeint_internal::int_traits<T>::isInt64 &&
                      safeint_internal::int_traits<U>::isUint64,
...
public:
  ...
  SAFE_INT_NODISCARD _CONSTEXPR14 static bool Multiply(T t, const U &u,
    ...
    if (LargeIntRegMultiply<std::int32_t, std::int64_t>::RegMultiply(
            (std::int32_t)t, u1, &tmp) &&
        SafeCastHelper<T, std::int32_t,
                       GetCastMethod<T, std::int32_t>::method>::Cast(tmp,
                                                                     ret)) {
  ...
  template <typename E>
  _CONSTEXPR14 static void MultiplyThrow(T t, const U &u,
                                         T &ret) SAFEINT_CPP_THROW {
    static_assert(safeint_internal::int_traits<U>::isInt64, "U must be Int64");
    std::int64_t u1 = u;
    std::int32_t tmp = 0;

    LargeIntRegMultiply<std::int32_t, std::int64_t>::template RegMultiplyThrow<
        E>((std::int32_t)t, u1, &tmp);
    SafeCastHelper<T, std::int32_t, GetCastMethod<T, std::int32_t>::method>::
        template CastThrow<E>(tmp, ret);
  }
...
public:
  enum {
    method =
        (safeint_internal::type_compare<T, U>::isBothUnsigned ? DivisionState_OK
         : (!std::numeric_limits<T>::is_signed &&
            std::numeric_limits<U>::is_signed)
             ? DivisionState_UnsignedSigned
         : (std::numeric_limits<T>::is_signed &&
...
public:
  SAFE_INT_NODISCARD _CONSTEXPR14 static SafeIntError
  Divide(const T &t, const U &u, T &result) SAFEINT_NOTHROW {
    if (u == 0)
      return SafeIntDivideByZero;
  ...
  template <typename E>
  _CONSTEXPR14 static void DivideThrow(const T &t, const U &u,
                                       T &result) SAFEINT_CPP_THROW {
    if (u == 0)
      E::SafeIntOnDivZero();
...
public:
  ...
  Divide(const T &t, const U &u, T &result) SAFEINT_NOTHROW {
    ...
    // it is always an error to try and divide an unsigned number by a negative
    // signed number unless u is bigger than t
...
public:
  ...
  Divide(const T &t, const U &u, T &result) SAFEINT_NOTHROW {
    ...
    // Test for t > 0
    // If t < 0, must explicitly upcast, or implicit upcast to ulong will cause
    // errors As it turns out, 32-bit division is about twice as fast, which
    // justifies the extra conditional
...
template <typename T, typename U>
class div_signed_uint64<T, U, true> // Value of u fits into an int32
{
public:
  SAFE_INT_NODISCARD _CONSTEXPR14 static T divide(T t, U u) {
    return (T)((std::int32_t)t / (std::int32_t)u);
  }
};
...
public:
  SAFE_INT_NODISCARD _CONSTEXPR14 static T divide(T t, U u) {
    return (T)((std::int64_t)t / (std::int64_t)u);
  }
};
...
public:
  SAFE_INT_NODISCARD _CONSTEXPR14 static SafeIntError
  Divide(const T &t, const std::uint64_t &u, T &result) SAFEINT_NOTHROW {
    static_assert(safeint_internal::int_traits<U>::isUint64,
                  "U must be Uint64");
    ...
    if (u <= (std::uint64_t)std::numeric_limits<T>::max()) {
      result =
          div_signed_uint64<T, U, sizeof(T) < sizeof(std::int64_t)>::divide(t,
                                                                            u);
    } else // Corner case
      if (t == std::numeric_limits<T>::min() &&
          u == (std::uint64_t)std::numeric_limits<T>::min()) {
  ...
  template <typename E>
  _CONSTEXPR14 static void DivideThrow(const T &t, const std::uint64_t &u,
                                       T &result) SAFEINT_CPP_THROW {
    static_assert(safeint_internal::int_traits<U>::isUint64,
                  "U must be Uint64");
...
template <typename T, typename U> class AdditionMethod {
public:
  enum {
    // unsigned-unsigned
    method =
...
public:
  SAFE_INT_NODISCARD _CONSTEXPR14 static bool
  Addition(const T &lhs, const U &rhs, T &result) SAFEINT_NOTHROW {
  ...
  template <typename E>
  _CONSTEXPR14 static void AdditionThrow(const T &lhs, const U &rhs,
                                         T &result) SAFEINT_CPP_THROW {
    // 16-bit or less unsigned addition
    std::int32_t tmp = lhs + rhs;
...
public:
  ...
  Addition(const T &lhs, const U &rhs, T &result) SAFEINT_NOTHROW {
    ...
    if (tmp <= (std::int32_t)std::numeric_limits<T>::max() &&
        tmp >= (std::int32_t)std::numeric_limits<T>::min()) {
      result = (T)tmp;
      return true;
    }
...
public:
  ...
  Addition(const T &lhs, const U &rhs, T &result) SAFEINT_NOTHROW {
    ...
    if (rhs < 0) {
      if (lhs >=
          ~(std::uint64_t)(rhs) + 1) // negation is safe, since rhs is 64-bit
      {
        result = (T)(lhs + rhs);
        return true;
      }
      ...
      // special case - rhs cannot be larger than 0x7fffffffffffffff, lhs cannot
      // be larger than 0xffffffff it is not possible for the operation above to
      // overflow, so just check max
      if (tmp <= std::numeric_limits<T>::max()) {
        result = (T)tmp;
        return true;
      }
...
public:
  ...
  Addition(const T &lhs, const U &rhs, T &result) SAFEINT_NOTHROW {
    ...
    if (AdditionHelper<
            ...
            AdditionState_CastInt64CheckOverflow>::Addition((std::int64_t)lhs,
                                                            (std::int64_t)rhs,
                                                            tmp) &&
  ...
  _CONSTEXPR14 static void AdditionThrow(const T &lhs, const U &rhs,
    ...
    AdditionHelper<std::int64_t, std::int64_t,
                   AdditionState_CastInt64CheckOverflow>::
        AdditionThrow<E>((std::int64_t)lhs, (std::int64_t)rhs, tmp);
...
public:
  SAFE_INT_NODISCARD _CONSTEXPR14 static bool
  Addition(const std::int64_t &lhs, const std::uint64_t &rhs,
           T &result) SAFEINT_NOTHROW {
    static_assert(safeint_internal::int_traits<T>::isInt64 &&
                      safeint_internal::int_traits<U>::isUint64,
                  "T must be Int64, U Uint64");
    // rhs is std::uint64_t, lhs std::int64_t
    // cast everything to unsigned, perform addition, then
    // cast back for check - this is done to stop optimizers from removing the
    // code
  ...
  template <typename E>
  _CONSTEXPR14 static void AdditionThrow(const std::int64_t &lhs,
...
public:
  ...
  Addition(const T &lhs, const U &rhs, T &result) SAFEINT_NOTHROW {
    ...
    if ((std::uint32_t)(rhs >> 32) == 0) {
      // Now it just happens to work out that the standard behavior does what we
      // want Adding explicit casts to show exactly what's happening here Note -
      // this is tweaked to keep optimizers from tossing out the code.
      std::uint32_t tmp = (std::uint32_t)rhs + (std::uint32_t)lhs;

      if ((std::int32_t)tmp >= lhs &&
          SafeCastHelper<
              T, std::int32_t,
              GetCastMethod<T, std::int32_t>::method>::Cast((std::int32_t)tmp,
                                                            result))
        return true;
    }
  ...
  _CONSTEXPR14 static void AdditionThrow(const T &lhs, const U &rhs,
    ...
    if ((std::uint32_t)(rhs >> 32) == 0) {
      // Now it just happens to work out that the standard behavior does what we
      // want Adding explicit casts to show exactly what's happening here
      std::uint32_t tmp = (std::uint32_t)rhs + (std::uint32_t)lhs;

      if ((std::int32_t)tmp >= lhs) {
        SafeCastHelper<T, std::int32_t,
                       GetCastMethod<T, std::int32_t>::method>::
            template CastThrow<E>((std::int32_t)tmp, result);
        return;
      }
    }
    E::SafeIntOnOverflow();
...
public:
  SAFE_INT_NODISCARD _CONSTEXPR14 static bool
  Subtract(const T &lhs, const U &rhs, T &result) SAFEINT_NOTHROW {
  ...
  template <typename E>
  _CONSTEXPR14 static void SubtractThrow(const T &lhs, const U &rhs,
                                         T &result) SAFEINT_CPP_THROW {
    // both are unsigned - easy case
    if (rhs <= lhs) {
      result = (T)(lhs - rhs);
...
public:
  SAFE_INT_NODISCARD _CONSTEXPR14 static bool
  Subtract(const T &lhs, const U &rhs, U &result) SAFEINT_NOTHROW {
    // both are unsigned - easy case
    // Except we do have to check for overflow - lhs could be larger than result
    // can hold
    if (rhs <= lhs) {
      T tmp = (T)(lhs - rhs);
      return SafeCastHelper<U, T, GetCastMethod<U, T>::method>::Cast(tmp,
                                                                     result);
    }
  ...
  template <typename E>
  _CONSTEXPR14 static void SubtractThrow(const T &lhs, const U &rhs,
                                         U &result) SAFEINT_CPP_THROW {
    // both are unsigned - easy case
    if (rhs <= lhs) {
      T tmp = (T)(lhs - rhs);
      SafeCastHelper<U, T, GetCastMethod<U, T>::method>::template CastThrow<E>(
          tmp, result);
      return;
    }
...
public:
  ...
  Subtract(const T &lhs, const U &rhs, T &result) SAFEINT_NOTHROW {
    ...
    if (SafeCastHelper<T, std::int32_t,
                       GetCastMethod<T, std::int32_t>::method>::Cast(tmp,
                                                                     result)) {
      result = (T)tmp;
      return true;
  ...
  _CONSTEXPR14 static void SubtractThrow(const T &lhs, const U &rhs,
    ...
    SafeCastHelper<T, std::int32_t, GetCastMethod<T, std::int32_t>::method>::
        template CastThrow<E>(tmp, result);
  }
};
...
public:
  SAFE_INT_NODISCARD _CONSTEXPR14 static bool
  Subtract(const U &lhs, const T &rhs, T &result) SAFEINT_NOTHROW {
    // both values are 16-bit or less
    // rhs is signed, so could end up increasing or decreasing
    std::int32_t tmp = lhs - rhs;

    return SafeCastHelper<T, std::int32_t,
                          GetCastMethod<T, std::int32_t>::method>::Cast(tmp,
                                                                        result);
  }

  template <typename E>
  _CONSTEXPR14 static void SubtractThrow(const U &lhs, const T &rhs,
                                         T &result) SAFEINT_CPP_THROW {
    // both values are 16-bit or less
    // rhs is signed, so could end up increasing or decreasing
    std::int32_t tmp = lhs - rhs;
...
public:
  ...
  Subtract(const T &lhs, const U &rhs, T &result) SAFEINT_NOTHROW {
    ...
    return SafeCastHelper<T, std::int64_t,
                          GetCastMethod<T, std::int64_t>::method>::Cast(tmp,
                                                                        result);
  }
  ...
  _CONSTEXPR14 static void SubtractThrow(const T &lhs, const U &rhs,
    ...
    SafeCastHelper<T, std::int64_t, GetCastMethod<T, std::int64_t>::method>::
        template CastThrow<E>(tmp, result);
  }
};
...
public:
  ...
  Subtract(const U &lhs, const T &rhs, T &result) SAFEINT_NOTHROW {
    ...
    if (rhs < 0) {
      ...
      tmp = lhs +
            (std::uint64_t)AbsValueHelper<T, GetAbsMethod<T>::method>::Abs(rhs);
    ...
    } else if ((std::uint64_t)rhs >
               lhs) // now both are positive, so comparison always works
    {
      // result is negative
      // implies that lhs must fit into T, and result cannot overflow
      // Also allows us to drop to 32-bit math, which is faster on a 32-bit
      // system
      result = (T)lhs - (T)rhs;
      return true;
    } else {
...
public:
  ...
  Subtract(const T &lhs, const U &rhs, T &result) SAFEINT_NOTHROW {
    ...
    // 2) lhs positive, rhs negative - equivalent to addition - result >= lhs or
    // error 3) lhs negative, rhs positive - check result <= lhs 4) lhs
    // negative, rhs negative - overflow not possible
...
public:
  SAFE_INT_NODISCARD _CONSTEXPR14 static bool
  isOverflowPositive(const T &rhs, const U &lhs, std::int64_t tmp) {
    return (tmp > std::numeric_limits<T>::max() || (rhs < 0 && tmp < lhs));
  }

  SAFE_INT_NODISCARD _CONSTEXPR14 static bool
  isOverflowNegative(const T &rhs, const U &lhs, std::int64_t tmp) {
...
public:
  ...
  Subtract(const U &lhs, const T &rhs, T &result) SAFEINT_NOTHROW {
    ...
    if (lhs >= 0) {
      ...
      if (subtract_corner_case_max<T, U,
                                   safeint_internal::int_traits<T>::isLT64Bit>::
              isOverflowNegative(rhs, lhs, tmp)) {
        return false;
      }
    }
...
public:
  ...
  Subtract(const T &lhs, const U &rhs, T &result) SAFEINT_NOTHROW {
    ...
    // lhs positive, rhs negative - additive case - check tmp >= lhs and tmp >
    // max int lhs negative, rhs positive - check tmp <= lhs and tmp < min int
    // lhs negative, rhs negative - addition cannot internally overflow, check
    // against max
...
public:
  ...
  Subtract(const U &lhs, const T &rhs, T &result) SAFEINT_NOTHROW {
    ...
    if ((std::int64_t)tmp <= std::numeric_limits<T>::max() &&
        (std::int64_t)tmp >= std::numeric_limits<T>::min()) {
      result = (T)(std::int64_t)tmp;
      return true;
    }
...
public:
  ...
  Subtract(const T &lhs, const U &rhs, T &result) SAFEINT_NOTHROW {
    ...
    const std::uint64_t AbsMinIntT =
        (std::uint64_t)std::numeric_limits<T>::max() + 1;

    if (lhs < 0) {
      if (rhs <=
          AbsMinIntT - AbsValueHelper<T, GetAbsMethod<T>::method>::Abs(lhs)) {
        result = (T)(lhs - rhs);
        return true;
      }
    } else {
  ...
  _CONSTEXPR14 static void SubtractThrow(const T &lhs, const U &rhs,
    ...
    // We need the absolute value of std::numeric_limits<T>::min()
    // This will give it to us without extraneous compiler warnings
    _CONSTEXPR11 std::uint64_t AbsMinIntT =
        (std::uint64_t)std::numeric_limits<T>::max() + 1;
...
public:
  SAFE_INT_NODISCARD _CONSTEXPR14 static bool
  Subtract(const std::int64_t &lhs, const std::uint64_t &rhs,
           std::int64_t &result) SAFEINT_NOTHROW {
    static_assert(safeint_internal::int_traits<T>::isInt64 &&
                      safeint_internal::int_traits<U>::isUint64,
                  "T must be Int64, U Uint64");
  ...
  template <typename E>
  _CONSTEXPR14 static void SubtractThrow(const std::int64_t &lhs,
                                         const std::uint64_t &rhs,
                                         T &result) SAFEINT_CPP_THROW {
    static_assert(safeint_internal::int_traits<T>::isInt64 &&
                      safeint_internal::int_traits<U>::isUint64,
...
public:
  // If lhs is negative, immediate problem - return must be positive, and
  // subtracting only makes it get smaller. If rhs > lhs, then it would also go
  // negative, which is the other case
  SAFE_INT_NODISCARD _CONSTEXPR14 static bool
  Subtract(const std::int64_t &lhs, const std::uint64_t &rhs,
           T &result) SAFEINT_NOTHROW {
    static_assert(safeint_internal::int_traits<T>::isUint64 &&
...
public:
  _CONSTEXPR11 static T Xor(T lhs, U rhs) SAFEINT_NOTHROW {
    return (T)(lhs ^ rhs);
  }
};
...
// External functions that can be used where you only need to check one
// operation non-class helper function so that you can check for a cast's
// validity and handle errors how you like
template <typename T, typename U>
SAFE_INT_NODISCARD _CONSTEXPR11 inline bool SafeCast(const T From,
                                                     U &To) SAFEINT_NOTHROW {
  return SafeCastHelper<U, T, GetCastMethod<U, T>::method>::Cast(From, To);
}

template <typename T, typename U>
SAFE_INT_NODISCARD _CONSTEXPR11 inline bool
SafeEquals(const T t, const U u) SAFEINT_NOTHROW {
  return EqualityTest<T, U, ValidComparison<T, U>::method>::IsEquals(t, u);
}

template <typename T, typename U>
SAFE_INT_NODISCARD _CONSTEXPR11 inline bool
SafeNotEquals(const T t, const U u) SAFEINT_NOTHROW {
  return !EqualityTest<T, U, ValidComparison<T, U>::method>::IsEquals(t, u);
}

template <typename T, typename U>
SAFE_INT_NODISCARD _CONSTEXPR11 inline bool
SafeGreaterThan(const T t, const U u) SAFEINT_NOTHROW {
  return GreaterThanTest<T, U, ValidComparison<T, U>::method>::GreaterThan(t,
                                                                           u);
}

template <typename T, typename U>
SAFE_INT_NODISCARD _CONSTEXPR11 inline bool
SafeGreaterThanEquals(const T t, const U u) SAFEINT_NOTHROW {
  return !GreaterThanTest<U, T, ValidComparison<U, T>::method>::GreaterThan(u,
                                                                            t);
}

template <typename T, typename U>
SAFE_INT_NODISCARD _CONSTEXPR11 inline bool
SafeLessThan(const T t, const U u) SAFEINT_NOTHROW {
  return GreaterThanTest<U, T, ValidComparison<U, T>::method>::GreaterThan(u,
                                                                           t);
}
...
template <typename T, typename U>
SAFE_INT_NODISCARD _CONSTEXPR11 inline bool
SafeLessThanEquals(const T t, const U u) SAFEINT_NOTHROW {
  return !GreaterThanTest<T, U, ValidComparison<T, U>::method>::GreaterThan(t,
                                                                            u);
}
...
template <typename T, typename U>
SAFE_INT_NODISCARD _CONSTEXPR11 inline bool
SafeModulus(const T &t, const U &u, T &result) SAFEINT_NOTHROW {
  return (ModulusHelper<T, U, ValidComparison<T, U>::method>::Modulus(
              t, u, result) == SafeIntNoError);
}

template <typename T, typename U>
SAFE_INT_NODISCARD _CONSTEXPR14_MULTIPLY inline bool
SafeMultiply(T t, U u, T &result) SAFEINT_NOTHROW {
  return MultiplicationHelper<
      T, U, MultiplicationMethod<T, U>::method>::Multiply(t, u, result);
}

template <typename T, typename U>
SAFE_INT_NODISCARD _CONSTEXPR11 inline bool
SafeDivide(T t, U u, T &result) SAFEINT_NOTHROW {
  return (DivisionHelper<T, U, DivisionMethod<T, U>::method>::Divide(
              t, u, result) == SafeIntNoError);
}

template <typename T, typename U>
SAFE_INT_NODISCARD _CONSTEXPR11 inline bool SafeAdd(T t, U u,
                                                    T &result) SAFEINT_NOTHROW {
  return AdditionHelper<T, U, AdditionMethod<T, U>::method>::Addition(t, u,
                                                                      result);
}

template <typename T, typename U>
SAFE_INT_NODISCARD _CONSTEXPR11 inline bool
SafeSubtract(T t, U u, T &result) SAFEINT_NOTHROW {
  return SubtractionHelper<T, U, SubtractionMethod<T, U>::method>::Subtract(
      t, u, result);
}

template <typename T>
SAFE_INT_NODISCARD _CONSTEXPR11 inline bool
SafeNegation(T t, T &result) SAFEINT_NOTHROW {
  return NegationHelper<T, std::numeric_limits<T>::is_signed>::Negative(t,
                                                                        result);
}
...
public:
  ...
  _CONSTEXPR14 SafeInt(const U &i) SAFEINT_CPP_THROW : m_int(0) {
    // m_int must be initialized to something to work with constexpr, because if
    // it throws, then m_int is unknown
    static_assert(safeint_internal::numeric_type<T>::isInt,
                  "Integer type required");
    // SafeCast will throw exceptions if i won't fit in type T

    SafeCastHelper<T, U, GetCastMethod<T, U>::method>::template CastThrow<E>(
        i, m_int);
  }
  ...
  template <typename U>
  _CONSTEXPR14 SafeInt<T, E> &
  operator=(const SafeInt<U, E> &rhs) SAFEINT_CPP_THROW {
    SafeCastHelper<T, U, GetCastMethod<T, U>::method>::template CastThrow<E>(
        rhs.Ref(), m_int);
    return *this;
  }
  ...
  _CONSTEXPR14 SafeInt<T, E> &
  operator=(const SafeInt<T, E> &rhs) SAFEINT_NOTHROW {
    m_int = rhs.m_int;
    return *this;
  }
  ...
  _CONSTEXPR11 operator bool() const SAFEINT_NOTHROW { return !!m_int; }

  _CONSTEXPR14 operator char() const SAFEINT_CPP_THROW {
    char val = 0;
    SafeCastHelper<char, T,
                   GetCastMethod<char, T>::method>::template CastThrow<E>(m_int,
                                                                          val);
    return val;
  }
  ...
  _CONSTEXPR14 operator signed char() const SAFEINT_CPP_THROW {
    signed char val = 0;
    SafeCastHelper<signed char, T, GetCastMethod<signed char, T>::method>::
        template CastThrow<E>(m_int, val);
    return val;
  }

  _CONSTEXPR14 operator unsigned char() const SAFEINT_CPP_THROW {
    unsigned char val = 0;
    SafeCastHelper<unsigned char, T, GetCastMethod<unsigned char, T>::method>::
        template CastThrow<E>(m_int, val);
    return val;
  }
  ...
  _CONSTEXPR14 operator short() const SAFEINT_CPP_THROW {
    short val = 0;
    SafeCastHelper<
        short, T, GetCastMethod<short, T>::method>::template CastThrow<E>(m_int,
                                                                          val);
    return val;
  }
  ...
  _CONSTEXPR14 operator unsigned short() const SAFEINT_CPP_THROW {
    ...
    SafeCastHelper<
        unsigned short, T,
        GetCastMethod<unsigned short, T>::method>::template CastThrow<E>(m_int,
                                                                         val);
    return val;
  }
  ...
  _CONSTEXPR14 operator int() const SAFEINT_CPP_THROW {
    int val = 0;
    SafeCastHelper<int, T,
                   GetCastMethod<int, T>::method>::template CastThrow<E>(m_int,
                                                                         val);
    return val;
  }
  ...
  _CONSTEXPR14 operator unsigned int() const SAFEINT_CPP_THROW {
    unsigned int val = 0;
    SafeCastHelper<unsigned int, T, GetCastMethod<unsigned int, T>::method>::
        template CastThrow<E>(m_int, val);
    return val;
  }

  // The compiler knows that int == std::int32_t
  // but not that long == std::int32_t, because on some systems, long ==
  // std::int64_t
  _CONSTEXPR14 operator long() const SAFEINT_CPP_THROW {
    long val = 0;
    SafeCastHelper<long, T,
                   GetCastMethod<long, T>::method>::template CastThrow<E>(m_int,
                                                                          val);
    return val;
  }
  ...
  _CONSTEXPR14 operator unsigned long() const SAFEINT_CPP_THROW {
    unsigned long val = 0;
    SafeCastHelper<unsigned long, T, GetCastMethod<unsigned long, T>::method>::
        template CastThrow<E>(m_int, val);
    return val;
  }
  ...
  _CONSTEXPR14 operator long long() const SAFEINT_CPP_THROW {
    long long val = 0;
    SafeCastHelper<long long, T, GetCastMethod<long long, T>::method>::
        template CastThrow<E>(m_int, val);
    return val;
  }
  ...
  _CONSTEXPR14 operator unsigned long long() const SAFEINT_CPP_THROW {
    ...
    SafeCastHelper<unsigned long long, T,
                   GetCastMethod<unsigned long long,
                                 T>::method>::template CastThrow<E>(m_int, val);
    return val;
  }

  _CONSTEXPR14 operator wchar_t() const SAFEINT_CPP_THROW {
    wchar_t val = 0;
    SafeCastHelper<wchar_t, T, GetCastMethod<wchar_t, T>::method>::
        template CastThrow<E>(m_int, val);
    return val;
  }
  ...
  // Apparently, only SOME compilers complain, and cl 14.00.50727.42 isn't one
  // of them Leave here in case we decide to backport this to an earlier
  // compiler
  _CONSTEXPR14 operator size_t() const SAFEINT_CPP_THROW {
    size_t val = 0;
    SafeCastHelper<size_t, T, GetCastMethod<size_t, T>::method>::
        template CastThrow<E>(m_int, val);
    return val;
  }
#endif
  ...
  _CONSTEXPR14 operator float() const SAFEINT_CPP_THROW {
    float val = 0.0;
    SafeCastHelper<
        float, T, GetCastMethod<float, T>::method>::template CastThrow<E>(m_int,
                                                                          val);
    return val;
  }
  ...
  _CONSTEXPR14 operator double() const SAFEINT_CPP_THROW {
    double val = 0.0;
    SafeCastHelper<double, T, GetCastMethod<double, T>::method>::
        template CastThrow<E>(m_int, val);
    return val;
  }
  _CONSTEXPR14 operator long double() const SAFEINT_CPP_THROW {
    long double val = 0.0;
    SafeCastHelper<long double, T, GetCastMethod<long double, T>::method>::
        template CastThrow<E>(m_int, val);
    return val;
  }
  ...
  // Unary operators
  _CONSTEXPR11 bool operator!() const SAFEINT_NOTHROW {
  ...
  _CONSTEXPR11 const SafeInt<T, E> &operator+() const SAFEINT_NOTHROW {
    return *this;
  }
  ...
  _CONSTEXPR14 SafeInt<T, E> operator-() const SAFEINT_CPP_THROW {
    ...
    return SafeInt<T, E>(NegationHelper<T, std::numeric_limits<T>::is_signed>::
                             template NegativeThrow<E>(m_int));
  }
  ...
  _CONSTEXPR14 SafeInt<T, E>
  operator++(int) SAFEINT_CPP_THROW // dummy arg to comply with spec
  {
    if (m_int != std::numeric_limits<T>::max()) {
  ...
  _CONSTEXPR11 SafeInt<T, E> operator~() const SAFEINT_NOTHROW {
  ...
  //
  // The base problem is that if the lhs and rhs inputs are different SafeInt
  // types it is not possible in this implementation to determine what type of
  // SafeInt should be returned. You have to let the class know which of the two
  // inputs need to be the return type by forcing the other value to the base
  // integer type.
  //
  // Note - as per feedback from Scott Meyers, I'm exploring how to get around
  // this. 3.0 update - I'm still thinking about this. It can be done with
  // template metaprogramming, but it is tricky, and there's a perf vs.
  // correctness tradeoff where the right answer is situational.
  //
  // The case of:
  //
  // SafeInt< T, E > i, j, k;
  // i = j op k;
  //
  // works just fine and no unboxing is needed because the return type is not
  // ambiguous.
  ...
  _CONSTEXPR14 SafeInt<T, E> operator%(U rhs) const SAFEINT_CPP_THROW {
    T result = 0;
    ModulusHelper<T, U, ValidComparison<T, U>::method>::template ModulusThrow<
        E>(m_int, rhs, result);
    return SafeInt<T, E>(result);
  }

  _CONSTEXPR14 SafeInt<T, E>
  operator%(SafeInt<T, E> rhs) const SAFEINT_CPP_THROW {
    T result = 0;
    ModulusHelper<T, T, ValidComparison<T, T>::method>::template ModulusThrow<
        E>(m_int, rhs, result);
    return SafeInt<T, E>(result);
  }
  ...
  _CONSTEXPR14_MULTIPLY SafeInt<T, E> operator*(U rhs) const SAFEINT_CPP_THROW {
    T ret(0);
    MultiplicationHelper<T, U, MultiplicationMethod<T, U>::method>::
        template MultiplyThrow<E>(m_int, rhs, ret);
  ...
  operator*(SafeInt<T, E> rhs) const SAFEINT_CPP_THROW {
    T ret(0);
    MultiplicationHelper<T, T, MultiplicationMethod<T, T>::method>::
        template MultiplyThrow<E>(m_int, (T)rhs, ret);
    return SafeInt<T, E>(ret);
  }
  ...
  _CONSTEXPR14 SafeInt<T, E> &operator*=(SafeInt<T, E> rhs) SAFEINT_CPP_THROW {
    MultiplicationHelper<T, T, MultiplicationMethod<T, T>::method>::
        template MultiplyThrow<E>(m_int, (T)rhs, m_int);
    return *this;
  }
  ...
  _CONSTEXPR14_MULTIPLY SafeInt<T, E> &operator*=(U rhs) SAFEINT_CPP_THROW {
    MultiplicationHelper<T, U, MultiplicationMethod<T, U>::method>::
        template MultiplyThrow<E>(m_int, rhs, m_int);
    return *this;
  }

  template <typename U>
  _CONSTEXPR14_MULTIPLY SafeInt<T, E> &
  operator*=(SafeInt<U, E> rhs) SAFEINT_CPP_THROW {
    MultiplicationHelper<T, U, MultiplicationMethod<T, U>::method>::
        template MultiplyThrow<E>(m_int, rhs.Ref(), m_int);
    return *this;
  }
  ...
  _CONSTEXPR14 SafeInt<T, E> operator/(U rhs) const SAFEINT_CPP_THROW {
    T ret(0);
    DivisionHelper<T, U, DivisionMethod<T, U>::method>::template DivideThrow<E>(
        m_int, rhs, ret);
    return SafeInt<T, E>(ret);
  }
  ...
  operator/(SafeInt<T, E> rhs) const SAFEINT_CPP_THROW {
    T ret(0);
    DivisionHelper<T, T, DivisionMethod<T, T>::method>::template DivideThrow<E>(
        m_int, (T)rhs, ret);
    return SafeInt<T, E>(ret);
  }
  ...
  operator+(SafeInt<T, E> rhs) const SAFEINT_CPP_THROW {
    T ret(0);
    AdditionHelper<T, T, AdditionMethod<T, T>::method>::template AdditionThrow<
        E>(m_int, (T)rhs, ret);
    return SafeInt<T, E>(ret);
  }
  ...
  _CONSTEXPR14 SafeInt<T, E> operator+(U rhs) const SAFEINT_CPP_THROW {
    T ret(0);
    AdditionHelper<T, U, AdditionMethod<T, U>::method>::template AdditionThrow<
        E>(m_int, rhs, ret);
    return SafeInt<T, E>(ret);
  }
  ...
  _CONSTEXPR14 SafeInt<T, E> operator-(U rhs) const SAFEINT_CPP_THROW {
    T ret(0);
    SubtractionHelper<
        T, U, SubtractionMethod<T, U>::method>::template SubtractThrow<E>(m_int,
                                                                          rhs,
                                                                          ret);
    return SafeInt<T, E>(ret);
  }
  ...
  operator-(SafeInt<T, E> rhs) const SAFEINT_CPP_THROW {
    T ret(0);
    SubtractionHelper<T, T, SubtractionMethod<T, T>::method>::
        template SubtractThrow<E>(m_int, (T)rhs, ret);
    return SafeInt<T, E>(ret);
  }
  ...
  _CONSTEXPR14 SafeInt<T, E> &operator-=(SafeInt<T, E> rhs) SAFEINT_CPP_THROW {
    SubtractionHelper<T, T, SubtractionMethod<T, T>::method>::
        template SubtractThrow<E>(m_int, (T)rhs, m_int);
    return *this;
  }

  template <typename U>
  _CONSTEXPR14 SafeInt<T, E> &operator-=(U rhs) SAFEINT_CPP_THROW {
    SubtractionHelper<T, U, SubtractionMethod<T, U>::method>::
        template SubtractThrow<E>(m_int, rhs, m_int);
    return *this;
  }
  ...
  _CONSTEXPR14 SafeInt<T, E> &operator-=(SafeInt<U, E> rhs) SAFEINT_CPP_THROW {
    SubtractionHelper<T, U, SubtractionMethod<T, U>::method>::
        template SubtractThrow<E>(m_int, (U)rhs, m_int);
    return *this;
  }
...
#define ShiftAssert(x) SAFEINT_ASSERT(x)
  ...
  template <typename U>
  _CONSTEXPR14 SafeInt<T, E>
  operator<<(SafeInt<U, E> bits) const SAFEINT_NOTHROW {
  ...
  // Bitwise &
  _CONSTEXPR14 SafeInt<T, E>
  operator&(SafeInt<T, E> rhs) const SAFEINT_NOTHROW {
  ...
  _CONSTEXPR14 SafeInt<T, E> operator&(U rhs) const SAFEINT_NOTHROW {
    ...
    return SafeInt<T, E>(
        BinaryAndHelper<T, U, BinaryMethod<T, U>::method>::And(m_int, rhs));
  }
  ...
  _CONSTEXPR14 SafeInt<T, E> &operator&=(SafeInt<U, E> rhs) SAFEINT_NOTHROW {
    m_int =
        BinaryAndHelper<T, U, BinaryMethod<T, U>::method>::And(m_int, (U)rhs);
    return *this;
  }
  ...
  _CONSTEXPR14 SafeInt<T, E> operator^(U rhs) const SAFEINT_NOTHROW {
    ...
    return SafeInt<T, E>(
        BinaryXorHelper<T, U, BinaryMethod<T, U>::method>::Xor(m_int, rhs));
  }
  ...
  _CONSTEXPR14 SafeInt<T, E> &operator^=(SafeInt<U, E> rhs) SAFEINT_NOTHROW {
    m_int =
        BinaryXorHelper<T, U, BinaryMethod<T, U>::method>::Xor(m_int, (U)rhs);
    return *this;
  }
  ...
  _CONSTEXPR14 SafeInt<T, E> operator|(U rhs) const SAFEINT_NOTHROW {
    return SafeInt<T, E>(
        BinaryOrHelper<T, U, BinaryMethod<T, U>::method>::Or(m_int, rhs));
  }
  ...
  SafeInt<T, E>
  Min(SafeInt<T, E> test,
      const T floor = std::numeric_limits<T>::min()) const SAFEINT_NOTHROW {
    T tmp = test < m_int ? (T)test : m_int;
    return tmp < floor ? floor : tmp;
  }

  SafeInt<T, E>
  Max(SafeInt<T, E> test,
      const T upper = std::numeric_limits<T>::max()) const SAFEINT_NOTHROW {
    T tmp = test > m_int ? (T)test : m_int;
  ...
  template <alignBits bits> const SafeInt<T, E> &Align() SAFEINT_CPP_THROW {
    // Zero is always aligned
    if (m_int == 0)
      return *this;

    // We don't support aligning negative numbers at this time
    // Can't align unsigned numbers on bitCount (e.g., 8 bits = 256, unsigned
    // char max = 255) or signed numbers on bitCount-1 (e.g., 7 bits = 128,
    // signed char max = 127). Also makes no sense to try to align on negative
    // or no bits.

    ShiftAssert(((std::numeric_limits<T>::is_signed &&
...
template <typename P>
_CONSTEXPR11 SafeInt<ptrdiff_t, SafeIntDefaultExceptionHandler>
SafePtrDiff(const P *p1, const P *p2) SAFEINT_CPP_THROW {
  return (p1 - p2);
}
...
_CONSTEXPR11 bool operator<(U lhs, SafeInt<T, E> rhs) SAFEINT_NOTHROW {
  return GreaterThanTest<T, U, ValidComparison<T, U>::method>::GreaterThan(
      (T)rhs, lhs);
}

template <typename T, typename U, typename E>
_CONSTEXPR11 bool operator<(SafeInt<T, E> lhs, U rhs) SAFEINT_NOTHROW {
  return GreaterThanTest<U, T, ValidComparison<U, T>::method>::GreaterThan(
      rhs, (T)lhs);
}

template <typename T, typename U, typename E>
_CONSTEXPR11 bool operator<(SafeInt<U, E> lhs,
                            SafeInt<T, E> rhs) SAFEINT_NOTHROW {
  return GreaterThanTest<T, U, ValidComparison<T, U>::method>::GreaterThan(
      (T)rhs, (U)lhs);
}
...
template <typename T, typename U, typename E>
_CONSTEXPR11 bool operator>(SafeInt<T, E> lhs,
                            SafeInt<U, E> rhs) SAFEINT_NOTHROW {
  return GreaterThanTest<T, U, ValidComparison<T, U>::method>::GreaterThan(
      (T)lhs, (U)rhs);
}
...
template <typename T, typename U, typename E>
_CONSTEXPR11 bool operator==(U lhs, SafeInt<T, E> rhs) SAFEINT_NOTHROW {
  return EqualityTest<T, U, ValidComparison<T, U>::method>::IsEquals((T)rhs,
                                                                     lhs);
}

template <typename T, typename U, typename E>
_CONSTEXPR11 bool operator==(SafeInt<T, E> lhs, U rhs) SAFEINT_NOTHROW {
  return EqualityTest<T, U, ValidComparison<T, U>::method>::IsEquals((T)lhs,
                                                                     rhs);
}
...
public:
  SAFE_INT_NODISCARD _CONSTEXPR14 static bool
  SignedCase(SafeInt<T, E> rhs, SafeInt<T, E> &result) SAFEINT_NOTHROW {
    if ((T)rhs == (T)-1) {
      result = 0;
      return true;
    }
...
public:
  _CONSTEXPR11 static bool
  SignedCase(SafeInt<T, E> /*rhs*/,
             SafeInt<T, E> & /*result*/) SAFEINT_NOTHROW {
    return false;
  }
};
...
public:
  SAFE_INT_NODISCARD _CONSTEXPR14 static bool
  ModulusSimpleCase(U lhs, SafeInt<T, E> rhs,
                    SafeInt<T, E> &result) SAFEINT_CPP_THROW {
    if (rhs != 0) {
      if (ModulusSignedCaseHelper<
              T, E, std::numeric_limits<T>::is_signed>::SignedCase(rhs, result))
        return true;
...
// Modulus
template <typename T, typename U, typename E>
_CONSTEXPR14 SafeInt<T, E> operator%(U lhs,
                                     SafeInt<T, E> rhs) SAFEINT_CPP_THROW {
  ...
  if (ModulusSimpleCaseHelper < T, U, E,
      (sizeof(T) == sizeof(U)) && ((bool)std::numeric_limits<T>::is_signed ==
                                   (bool)std::numeric_limits<U>::is_signed) >
                                      ::ModulusSimpleCase(lhs, rhs, result))
    return result;
...
template <typename T, typename U, typename E>
_CONSTEXPR14_MULTIPLY SafeInt<T, E>
operator*(U lhs, SafeInt<T, E> rhs) SAFEINT_CPP_THROW {
  T ret(0);
  MultiplicationHelper<T, U, MultiplicationMethod<T, U>::method>::
      template MultiplyThrow<E>((T)rhs, lhs, ret);
  return SafeInt<T, E>(ret);
}
...
public:
  // sizeof(T) == 4
  SAFE_INT_NODISCARD _CONSTEXPR14 static U div(T rhs, U lhs) {
    return lhs / (U)(~(std::uint32_t)(T)rhs + 1);
  }
};
...
public:
  SAFE_INT_NODISCARD _CONSTEXPR14 static U div(T rhs, U lhs) {
    return lhs / (U)(~(std::uint64_t)(T)rhs + 1);
  }
};
...
public:
  SAFE_INT_NODISCARD static bool
  NegativeCornerCase(U lhs, SafeInt<T, E> rhs,
                     SafeInt<T, E> &result) SAFEINT_CPP_THROW {
...
public:
  SAFE_INT_NODISCARD _CONSTEXPR14 static bool
  DivisionCornerCase1(U lhs, SafeInt<T, E> rhs,
                      SafeInt<T, E> &result) SAFEINT_CPP_THROW {
    if ((T)rhs > 0) {
    ...
    if ((T)rhs != 0) {
      if (DivisionNegativeCornerCaseHelper < T, U, E,
          sizeof(U) >= 4 &&
...
public:
  SAFE_INT_NODISCARD _CONSTEXPR14 static bool Value(T &) { return false; }
};
...
public:
  SAFE_INT_NODISCARD _CONSTEXPR14 static bool
  DivisionCornerCase2(U lhs, SafeInt<T, E> rhs,
                      SafeInt<T, E> &result) SAFEINT_CPP_THROW {
...
_CONSTEXPR14 SafeInt<T, E> operator/(U lhs,
  ...
  if (DivisionCornerCaseHelper<
          T, U, E,
          (int)DivisionMethod<U, T>::method ==
              (int)DivisionState_UnsignedSigned>::DivisionCornerCase1(lhs, rhs,
                                                                      result))
    return result;

  if (DivisionCornerCaseHelper2<
          T, U, E, safeint_internal::type_compare<T, U>::isBothSigned>::
          DivisionCornerCase2(lhs, rhs, result))
    return result;

  // Otherwise normal logic works with addition of bounds check when casting
  // from U->T
  U ret = 0;
  DivisionHelper<U, T, DivisionMethod<U, T>::method>::template DivideThrow<E>(
      lhs, (T)rhs, ret);
  return SafeInt<T, E>(ret);
}
...
_CONSTEXPR14 SafeInt<T, E> operator+(U lhs,
                                     SafeInt<T, E> rhs) SAFEINT_CPP_THROW {
  T ret(0);
  AdditionHelper<T, U, AdditionMethod<T, U>::method>::template AdditionThrow<E>(
      (T)rhs, lhs, ret);
  return SafeInt<T, E>(ret);
}
...
_CONSTEXPR14 SafeInt<T, E> operator-(U lhs,
                                     SafeInt<T, E> rhs) SAFEINT_CPP_THROW {
  T ret(0);
  SubtractionHelper<U, T, SubtractionMethod2<U, T>::method>::
      template SubtractThrow<E>(lhs, rhs.Ref(), ret);
...
_CONSTEXPR14 T &operator-=(T &lhs, SafeInt<U, E> rhs) SAFEINT_CPP_THROW {
  T ret(0);
  SubtractionHelper<
      T, U, SubtractionMethod<T, U>::method>::template SubtractThrow<E>(lhs,
                                                                        (U)rhs,
                                                                        ret);
  lhs = ret;
  return lhs;
...
_CONSTEXPR14 T &operator*=(T &lhs, SafeInt<U, E> rhs) SAFEINT_CPP_THROW {
  T ret(0);
  MultiplicationHelper<T, U, MultiplicationMethod<T, U>::method>::
      template MultiplyThrow<E>(lhs, (U)rhs, ret);
  lhs = ret;
  return lhs;
}
...
_CONSTEXPR14 T &operator%=(T &lhs, SafeInt<U, E> rhs) SAFEINT_CPP_THROW {
  T ret(0);
  ModulusHelper<T, U, ValidComparison<T, U>::method>::template ModulusThrow<E>(
      lhs, (U)rhs, ret);
  lhs = ret;
  return lhs;
}
...
T *&operator+=(T *&lhs, SafeInt<U, E> rhs) SAFEINT_CPP_THROW {
  ...
  // Note: this doesn't really make sense as a constexpr, but cannot be because
  // of the reinterpret_cast
  ...
  lhs = reinterpret_cast<T *>(
      (size_t)(ptr_val + (ptrdiff_t)(SafeInt<ptrdiff_t, E>(rhs) * sizeof(T))));
  return lhs;
}
...
// Left shift
template <typename T, typename U, typename E>
_CONSTEXPR14 SafeInt<U, E> operator<<(U lhs,
                                      SafeInt<T, E> bits) SAFEINT_NOTHROW {
...
_CONSTEXPR11 SafeInt<T, E> operator&(U lhs, SafeInt<T, E> rhs) SAFEINT_NOTHROW {
  return SafeInt<T, E>(
      BinaryAndHelper<T, U, BinaryMethod<T, U>::method>::And((T)rhs, lhs));
}
...
_CONSTEXPR11 SafeInt<T, E> operator^(U lhs, SafeInt<T, E> rhs) SAFEINT_NOTHROW {
  return SafeInt<T, E>(
      BinaryXorHelper<T, U, BinaryMethod<T, U>::method>::Xor((T)rhs, lhs));
}
...
_CONSTEXPR11 SafeInt<T, E> operator|(U lhs, SafeInt<T, E> rhs) SAFEINT_NOTHROW {
  return SafeInt<T, E>(
      BinaryOrHelper<T, U, BinaryMethod<T, U>::method>::Or((T)rhs, lhs));
}
